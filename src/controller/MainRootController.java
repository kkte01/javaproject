...
public class MainRootController implements Initializable{
	//fx:id 매치 시키기
	@FXML private ImageView imgTrainer;
	@FXML private ImageView imgPoket;
	@FXML private Label labelTrainer;
	@FXML private Button btnRandomPoket;
	@FXML private Button btnRandomName;
	@FXML private Button btnChangeName;
	@FXML private Button btnChangeImage;
	@FXML private Label labelNotice;
	@FXML private Button btnClose;
	@FXML private Button btnBook;
	@FXML private Button btnMyPoket;
	@FXML private Button btnGlow;
	@FXML private Button btnToday;
	@FXML private BarChart mainXYchart;
	@FXML private Label lblMonName;
	public Stage stage;
	...
	private void loadRamdomPhoto() {
		/*
		 * BookDAO bookDAO = new BookDAO();
		 * ArrayList<PoketmonBook1>arrayList=bookDAO.getJoinLoadList(); int i
		 * =(int)Math.random()*(arrayList.size())-1; PoketmonBook1 poketmon =
		 * arrayList.get(i); XYChart.Series atk = new XYChart.Series();
		 * atk.setName("HP"); ObservableList atkList =
		 * FXCollections.observableArrayList(); for(int i1 = 0; i1<arrayList.size();
		 * i1++) { atkList.add(new
		 * XYChart.Data(atk.getName(),Integer.parseInt(poketmon.getHp()))); }
		 * atk.setData(atkList); mainXYchart.getData().add(atk);
		 */
	...
	private void handlelblNoticeAction(MouseEvent e) {
		...
		btnCmdLogin.setOnAction(event ->{
				if(pwField1.getText().equals("123456")) {
					...
					//취소버튼 이벤트
					btnCancel.setOnAction(event2 -> {cmd.close();noti.close();});
					//등록 버튼 이벤트
	...
	private void handleBtnBookAction(ActionEvent e) {
		...
		// 도감 컬럼에 들어가는 정보 입력
			...
			tbcImage.setCellValueFactory(new PropertyValueFactory<>("image2"));
			...
			// DB 연동
			BookDAO bookDAO = new BookDAO();
			ArrayList<PoketmonBook1>arrayList = bookDAO.getTotalLoadList();
				...
				tblBook.setOnMouseClicked(event ->{
					...
					Parent book = null;
					int tableSelctedIndex=tblBook.getSelectionModel().getSelectedIndex();
						...
						try {
							...
							Label lblInforGroup=(Label) book.lookup("#lblInforGroup");
